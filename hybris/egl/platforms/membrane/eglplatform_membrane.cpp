...
public:
	...
	bool allocate(unsigned int w, unsigned int h, unsigned int fmt,
		...
		int ret = hybris_gralloc_allocate(w, h, fmt, (uint32_t)usg, &handle,
									(uint32_t *)&stride);
		if (ret != 0) {
			HYBRIS_ERROR("Failed to allocate buffer: %d\n", ret);
			return false;
		}
		...
		if (meta_size > 0) {
			...
			if (m_meta_fd >= 0) {
				if (ftruncate(m_meta_fd, meta_size) == -1) {
					HYBRIS_ERROR("Failed to ftruncate meta buffer: %s\n", strerror(errno));
					close(m_meta_fd);
					m_meta_fd = -1;
					return false;
				}
				if (write(m_meta_fd, &nh->data[nh->numFds], meta_size) != meta_size) {
					HYBRIS_ERROR("Failed to write meta buffer: %s\n", strerror(errno));
					close(m_meta_fd);
					m_meta_fd = -1;
					return false;
				}
...
public:
	...
	virtual int queueBuffer(BaseNativeWindowBuffer *buffer,
		...
		if (fenceFd >= 0) {
			...
			if (!symbols_resolved) {
				eglCreateSyncKHR = (PFNEGLCREATESYNCKHRPROC)eglplatformcommon_eglGetProcAddress("eglCreateSyncKHR");
				eglWaitSyncKHR = (PFNEGLWAITSYNCKHRPROC)eglplatformcommon_eglGetProcAddress("eglWaitSyncKHR");
				eglDestroySyncKHR = (PFNEGLDESTROYSYNCKHRPROC)eglplatformcommon_eglGetProcAddress("eglDestroySyncKHR");
				symbols_resolved = true;
			}

			bool handled = false;
			EGLDisplay dpy = eglplatformcommon_eglGetCurrentDisplay();
			if (dpy != EGL_NO_DISPLAY && eglCreateSyncKHR && eglWaitSyncKHR && eglDestroySyncKHR) {
				int fd_dup = dup(fenceFd);
				EGLint attribs[] = {
					EGL_SYNC_NATIVE_FENCE_FD_ANDROID, fd_dup,
					EGL_NONE
				};
		...
		if (mnb->getWlBuffer()) {
			...
			HYBRIS_ERROR("Failed to create wl_buffer for queue");
			mnb->setBusy(0);
			return -1;
		}
...
private:
	...
	void createWlBuffer(MembraneNativeWindowBuffer *mnb) {
		...
		struct wl_buffer *wl_buf = zwp_linux_buffer_params_v1_create_immed(
			params, mnb->width, mnb->height, 0x34325258, 0);
		...
		if (wl_buf) {
			...
			HYBRIS_ERROR("Failed to create wl_buffer from params");
		}
	}
...
membranews_CreateWindow(EGLNativeWindowType win, struct _EGLDisplay *display) {
	...
	if (!dpy->dmabuf) {
		...
		if (!dpy->dmabuf) {
			HYBRIS_ERROR("zwp_linux_dmabuf_v1 not supported");
