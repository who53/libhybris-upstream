		...
		const EGLint *attrib_list)
{
	...
	EGLContext ctx = (*_eglCreateContext)(hybris_egl_get_real_display(dpy), config, share_context, attrib_list);

	if (ctx == EGL_NO_CONTEXT && config == EGL_NO_CONFIG_KHR) {
        ctx = (*_eglCreateContext)(hybris_egl_get_real_display(dpy), hybris_egl_choose_best_config(dpy, _egl_context_client_version), share_context, attrib_list);
    }
                              ...
                              EGLint *num_modifiers)
{
	...
	if (format != 0x34325258) // DRM_FORMAT_XRGB8888
		return EGL_FALSE;
